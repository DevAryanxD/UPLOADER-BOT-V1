...
class Database:
    ...
    def new_user(self, id):
        return dict(
            ...
            thumbnail=None,
            caption=None,
            upload_channel=None,  # New field for upload channel
            scrape_channel=None   # New field for scrape channel
        )
    ...
    async def set_upload_channel(self, id, channel_id):
        await self.col.update_one({'id': id}, {'$set': {'upload_channel': channel_id}})

    async def get_upload_channel(self, id):
        user = await self.col.find_one({'id': int(id)})
        return user.get('upload_channel', None)

    async def set_scrape_channel(self, id, channel_id):
        await self.col.update_one({'id': id}, {'$set': {'scrape_channel': channel_id}})

    async def get_scrape_channel(self, id):
        user = await self.col.find_one({'id': int(id)})
        return user.get('scrape_channel', None)
...
db = Database(Config.DATABASE_URL, "UploaderBot")